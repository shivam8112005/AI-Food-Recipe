...
export default function EditRecipe() {
    ...
    useEffect(()=>{
        const getData=async()=>{await axios.get(`http://localhost:5000/recipe/${id}`).then((res)=>{
            let res1=res.data['recipe'];
            console.log(res1);
            
            setRecipeData({
                title:res1.title,
                ingredients:res1.ingredients.join(","),
    ...
    const onHandleSubmit = async (e) => {
        ...
        await axios.put(`http://localhost:5000/recipe/${id}`, recipeData, {
            ...
            .then(() => {
                ...
                navigate("/myRecipe")}).catch((err)=>{console.log(err," hellooooooo")});
